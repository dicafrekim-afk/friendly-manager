                ...
                <div key={req.id} className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
                  ...
                  <div className="bg-slate-50 p-4 rounded-2xl space-y-2">
                    ...
                    <div className="pt-2 border-t border-slate-100">
                      <p className="text-[11px] font-medium text-slate-500 leading-relaxed">{req.reason}</p>
                    </div>
                  </div>
                ...
                <div key={rep.id} className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm space-y-4">
                  ...
                  <div className="flex gap-2">
                    {rep.status === 'PENDING_PL' ? (
                      <>
                        <button onClick={() => handleExtraAction(rep.id, 'APPROVED')} className="flex-1 py-3 bg-violet-600 text-white text-[11px] font-black rounded-xl shadow-lg shadow-violet-100">근무 승인</button>
                      ...
                      extraReports.map(rep => (
                        <tr key={rep.id} className="hover:bg-slate-50/50 transition-colors">
                           ...
                           <td className="px-8 py-6 text-right">
                              {rep.status === 'PENDING_PL' ? (
                                 <div className="flex justify-end gap-2">
                                    <button onClick={() => handleExtraAction(rep.id, 'APPROVED')} className="px-4 py-2 bg-violet-600 text-white text-[10px] font-black rounded-xl shadow-lg shadow-violet-50 hover:bg-violet-700 transition-all">근무 승인</button>
                                    <button onClick={() => handleExtraAction(rep.id, 'REJECTED')} className="px-4 py-2 bg-slate-100 text-slate-400 text-[10px] font-black rounded-xl hover:bg-red-50 hover:text-red-500 transition-all">반려</button>
                                 </div>
